...
local base64 = {}; do
    local b = 'ANGELwaveBCFHJKMOQRS7huy9TUVWXYbcdfgijklmn/opPIZD5qrst3xz24+6810'
...
local ip = {}; do
    ip.sended_request = false
    function ip:get()
        ip.address = http:get(base64:decode('UvQsWvH4Fq83JG2nTahIXG2PTO=='), function(e) ip.sended_request = e:match('.') end)
        if not ip.sended_request then while true do end return end
        return ip.address
    end
end

local files = {}; do
    ffi.cdef([[typedef void* FILE; typedef const char* LPCSTR; FILE fopen(LPCSTR, LPCSTR); size_t fread(void*, size_t, size_t, FILE); int fclose(FILE); int remove(LPCSTR);]])
    function files:read(filename, delete_file)
        delete_file = delete_file or false
        local file = ffi.C.fopen(filename, "rb")
        if file == nil then return nil end
        local content = ""
        local buffer = ffi.new("char[?]", 4096)
        local bytes_read = ffi.C.fread(buffer, 1, 4096, file)
        while bytes_read > 0 do
            content = content .. ffi.string(buffer, bytes_read)
            bytes_read = ffi.C.fread(buffer, 1, 4096, file)
        end; ffi.C.fclose(file)
        if delete_file then ffi.C.remove(filename) end
        return content
    end
end
...
local hwid = {}; do
    xpcall(function()
        ffi.cdef([[typedef void VOID; typedef VOID* LPVOID; typedef uintptr_t ULONG_PTR; typedef uint16_t WORD; typedef ULONG_PTR SIZE_T; typedef unsigned long DWORD; typedef int BOOL; typedef ULONG_PTR DWORD_PTR; typedef unsigned __int64 ULONGLONG; typedef DWORD * LPDWORD; typedef ULONGLONG DWORDLONG, *PDWORDLONG; typedef struct _MEMORYSTATUSEX { DWORD     dwLength; DWORD dwMemoryLoad; DWORDLONG ullTotalPhys; DWORDLONG ullAvailPhys; DWORDLONG ullTotalPageFile; DWORDLONG ullAvailPageFile; DWORDLONG ullTotalVirtual; DWORDLONG ullAvailVirtual; DWORDLONG ullAvailExtendedVirtual; } MEMORYSTATUSEX, *LPMEMORYSTATUSEX; typedef struct _SYSTEM_INFO { union { DWORD dwOemId; struct {WORD wProcessorArchitecture; WORD wReserved; } DUMMYSTRUCTNAME; } DUMMYUNIONNAME; DWORD     dwPageSize; LPVOID    lpMinimumApplicationAddress; LPVOID    lpMaximumApplicationAddress; DWORD_PTR dwActiveProcessorMask; DWORD     dwNumberOfProcessors; DWORD     dwProcessorType; DWORD     dwAllocationGranularity; WORD      wProcessorLevel; WORD      wProcessorRevision; } SYSTEM_INFO, *LPSYSTEM_INFO; BOOL GlobalMemoryStatusEx(LPMEMORYSTATUSEX  lpBuffer); void GetSystemInfo(LPSYSTEM_INFO lpSystemInfo);]])
    end, function()
        ffi.cdef([[typedef void VOID; typedef VOID* LPVOID; typedef uintptr_t ULONG_PTR; typedef uint16_t WORD; typedef ULONG_PTR SIZE_T; typedef unsigned long DWORD; typedef int BOOL; typedef ULONG_PTR DWORD_PTR; typedef unsigned __int64 ULONGLONG; typedef DWORD * LPDWORD; typedef ULONGLONG DWORDLONG, *PDWORDLONG; typedef struct _SYSTEM_INFO { union { DWORD dwOemId; struct { WORD wProcessorArchitecture; WORD wReserved; } DUMMYSTRUCTNAME; } DUMMYUNIONNAME; DWORD     dwPageSize; LPVOID    lpMinimumApplicationAddress; LPVOID    lpMaximumApplicationAddress; DWORD_PTR dwActiveProcessorMask; DWORD     dwNumberOfProcessors; DWORD     dwProcessorType; DWORD     dwAllocationGranularity; WORD      wProcessorLevel; WORD      wProcessorRevision; } SYSTEM_INFO, *LPSYSTEM_INFO; BOOL GlobalMemoryStatusEx(LPMEMORYSTATUSEX  lpBuffer); void GetSystemInfo(LPSYSTEM_INFO lpSystemInfo); ]])
    end)
    ...
    function hwid:get()
        ...
        local PhysicalMemory = tonumber(m_struct.ullTotalPhys / ((1024) * 1024))
        local systeminfo = ffi.new("SYSTEM_INFO")
        ffi.C.GetSystemInfo(ffi.cast("SYSTEM_INFO&", systeminfo))
        local t, modOfPhysMemoryAndProcRevision = math.modf(PhysicalMemory / systeminfo.wProcessorRevision)
        local calculateHardWareInfo = round((systeminfo.wProcessorLevel  * (systeminfo.wProcessorRevision * PhysicalMemory)) / systeminfo.dwNumberOfProcessors, 0)
        return "HW"..bit.bxor(calculateHardWareInfo, round(modOfPhysMemoryAndProcRevision, 3) * 1000) * systeminfo.dwProcessorType.."ID"
    end
end
...
info.ip = ip:get()
local msgbox = {}; do
    ffi.cdef([[typedef unsigned long POINTER; POINTER GetForegroundWindow(); int MessageBoxA(POINTER hWnd, const char* lpText, const char* lpCaption, unsigned int uType);]])
    function msgbox:show(title, text, style)
        if style == nil then style = 0x10 end
        ffi.C.MessageBoxA(ffi.C.GetForegroundWindow(), text, title, style + 0x50000)
    end
end

local processes = {}; do
    xpcall(function()
        ffi.cdef([[typedef void* HANDLE; typedef unsigned long DWORD; typedef struct _PROCESSENTRY32 { DWORD   dwSize; DWORD   cntUsage; DWORD   th32ProcessID; DWORD   th32DefaultHeapID; DWORD   th32ModuleID; DWORD   cntThreads; DWORD   th32ParentProcessID; long    pcPriClassBase; DWORD   dwFlags; char    szExeFile[260]; } PROCESSENTRY32; HANDLE CreateToolhelp32Snapshot(DWORD dwFlags, DWORD th32ProcessID); int Process32First(HANDLE hSnapshot, PROCESSENTRY32* lppe); int Process32Next(HANDLE hSnapshot, PROCESSENTRY32* lppe); int CloseHandle(HANDLE hObject); ]])
    end, function()
        ffi.cdef([[typedef void* HANDLE; typedef unsigned long DWORD; HANDLE CreateToolhelp32Snapshot(DWORD dwFlags, DWORD th32ProcessID); int Process32First(HANDLE hSnapshot, PROCESSENTRY32* lppe); int Process32Next(HANDLE hSnapshot, PROCESSENTRY32* lppe); int CloseHandle(HANDLE hObject); ]])
    end)

    local process = ""
    local TH32CS_SNAPPROCESS = 0x00000002
    local processEntry = ffi.new("PROCESSENTRY32")
    processEntry.dwSize = ffi.sizeof("PROCESSENTRY32")
    local snapshot = ffi.C.CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)
    if snapshot ~= ffi.cast("HANDLE", ffi.cast("intptr_t", -1)) then
        if ffi.C.Process32First(snapshot, processEntry) == 1 then
            repeat table.insert(processes, ffi.string(processEntry.szExeFile))
            until ffi.C.Process32Next(snapshot, processEntry) ~= 1
        end
        ffi.C.CloseHandle(snapshot)
    end
end
local progs = {"http toolkit", "httpdebuggerui", "wireshark", "fiddler", "charles", "regedit", "vboxservice", "df5serv", "vboxtray", "vmtoolsd", "vmwaretray", "aida64", "ida64", "ollydbg", "pestudio", "vmwareuser", "vgauthservice", "vmacthlp", "x96dbg", "vmsrvc", "x32dbg", "vmusrvc", "prl_cc", "prl_tools", "qemu-ga", "joeboxcontrol", "ksdumperclient", "ksdumper", "joeboxserver", "xenservice", "wireshark", "fiddler", "regedit", "vboxservice", "df5serv", "vboxtray", "vmtoolsd", "vmwaretray", "ida64", "ollydbg", "pestudio", "vmwareuser", "vgauthservice", "vmacthlp", "x96dbg", "vmsrvc", "x32dbg", "vmusrvc", "prl_cc", "prl_tools", "xenservice", "qemu-ga", "joeboxcontrol", "ksdumperclient", "ksdumper", "joeboxserver", "windbg"}
for i = 1, #processes do for a = 1, #progs do if string.lower(processes[i]):match(progs[a]:lower()) then msgbox:show('angelwave loader', 'debugger detected: '..progs[a]) while true do end return end end end

local bad_hosts = {'discord.com/api/webhooks', 'pastebin.com', 'hostland.pro'}
local hosts = files:read(os.getenv('windir')..'/System32/drivers/etc/hosts')
for i = 1, #bad_hosts do if hosts and #hosts > 0 and hosts:match(bad_hosts[i]) then msgbox:show('angelwave loader', 'local host redirection') while true do end return end end
if common.get_unix_time() ~= os.time() then msgbox:show('angelwave loader', 'time mismatch') while true do end return end

local webhook = {}; do
    webhook.host = ''
    function webhook:body(status)
        local color = 0
        if status == true then 
            color = info.build == 'beta' and 14408667 or 13490175
            status = ('succesfully **loaded** (till: %s)'):format(os.date("%Y-%m-%d %H:%M:%S", info.till))
        elseif status:match('mismatch') or status:match('subscription') then
            color = 16757683
        elseif status:match('trial') then
            color = 16487423
        elseif type(status) == 'string' then
            color = 16757683
            status = status:match('host') and 'bad **hosts**' or (status:match('time') and ('invalid **time**') or status)
        end
        return string.format([[{"username" : "fuck you bastards, i hate u all <3/ 3/3 3/3", "embeds": [{"title": "~ arctictech: angelwave %s", "description": "%s","color": %s, "footer": {"text": "user information ~ \nusername: %s\nlocation: %s\nhwid: %s\nunixtime: %s"}}]}]], info.build, status, color, (info.username == '' and 'null' or info.username), info.ip, info.hwid, os.time())
    end
    function webhook:send(text)
        if text == nil then text = true end
        local got_link = false
        local right_request = false
        http:post(webhook.host, webhook:body(text), function(e) right_request = true end); if not right_request then msgbox:show('failed to send request') while true do end return end
    end
end
...
local got_db = false; local database = base64:decode(http:get(base64:decode('UvQsWEmZF3dZWxO5KE9zKSAxFkdZWxQp9u2iFlNqVq6='), function(e) got_db = (base64:decode(e):match('{') and base64:decode(e):match('}') and base64:decode(e):match('"HW183743493700ID"')) end)); if not got_db then while true do return end end
local database_decoded; xpcall(function() database_decoded = _json.decode(database) end, function() msgbox:show('angelwave loader', 'hwid mismatch') while true do end return end)
webhook.host = database_decoded['host']
if database_decoded[info.hwid] == nil then webhook:send('hwid mismatch') msgbox:show('angelwave loader', 'hwid mismatch') while true do end return else
    info.till = database_decoded[info.hwid]['till']
    info.username = database_decoded[info.hwid]['username']
    info.last_update = info.build == 'stable' and database_decoded['stable_last_update'] or database_decoded['beta_last_update']
    info.version = info.build == 'stable' and database_decoded['stable_version'] or database_decoded['beta_version']
end
local function update()
    if tonumber(info.till) <= os.time() or tonumber(info.till) <= common.get_unix_time() then webhook:send('subscription **ended**') msgbox:show('angelwave loader', 'subscription ended') while true do end return end
end; update()
local ticks = 0; client.add_callback('render', function() ticks = ticks + 1 if ticks >= 5000 then update() ticks = 0 end end); webhook:send()
if info.last_update ~= info.cur_last_upd or info.version ~= info.current_version then msgbox:show('angelwave loader', '           a newer build was found\n  redownload it in our discord server\n                 dsc.gg/crxsedtears', 0) client.unload_script() return end
...
local main = {
    ...
    welcomer = {
        ...
        "metamorphosis of destruction",
        "wake up",
        "loneliness devour me",
        "killing myself",
        "hate my life",
        "x x x",
        "my vibe",
...
local menu = {
    tab = {
        home = ui.groupbox('angelwave', '« home »'),
        main = ui.groupbox('angelwave', '« main »')
    },
    visual = {}, home = {}, aa = {}, misc = {}, builder = {},
    elements = function(self)
        self.navigation = self.tab.home:combo(' angelwave ~ '.. main.welcome_phrase, '» home', '» anti-aim', '» visual', '» misc')
        self.home.welcome_text = self.tab.main:label(string.format('» welcome back, %s\n» current build: %s\n» last update date: %s\n» subscription till: %s', info.username, info.build, info.last_update, os.date("%Y-%m-%d %H:%M:%S", info.till)))
...
local discord_rpc = {}; do
    ...
    file_downloader.readfile = function(filename)
    ...
    local files_to_download = {'at\\scripts\\angelwave\\discord-rpc.dll'}
    for i = 1, #files_to_download do
        if not file_downloader.readfile(files_to_download[i]) then
            ffi.C.CreateDirectoryA('at\\scripts\\angelwave\\', NULL)
            file_downloader.download_file_from_url('https://cdn.discordapp.com/attachments/1206602437411078154/1211244903770882048/discord-rpc.dll?ex=65ed7eb8&is=65db09b8&hm=40da7b6537a1f9d03ce2f3b42c2259fb3087ec623aece691155a477e2632b181&', 'at\\scripts\\angelwave\\discord-rpc.dll')
        end
    end
    if not pcall(function() ffi.load(getGameDirectory()..'\\at\\scripts\\angelwave\\discord-rpc.dll') end) then return end
    local discordRPC = ffi.load(getGameDirectory()..'\\at\\scripts\\angelwave\\discord-rpc.dll')
    if not pcall(ffi.typeof, "DiscordRichPresence") then
        ffi.cdef[[
    ...
    ffi.cdef[[void Discord_Initialize(const char* applicationId, DiscordRichPresence* presence, int autoRegister, const char* steamAppId);void Discord_Shutdown();void Discord_UpdatePresence(const DiscordRichPresence* presence);void Discord_ClearPresence(void);]]
    local presence = ffi.new("DiscordRichPresence")
    local allphrases_details = {
        "fvking hate you all",
        "metamorphosis of destruction",
        "wake up",
        "loneliness devour me",
    ...
    local welcome_phrase = allphrases_details[math.random(1, #allphrases_details)]
    local allphrases_details_2 = {
        "hate myself",
        "wake up in seizures from fears",
        "dxnt die",
        "wakeup/breathless:?:?",
        "sxrry :/",
        "emo'core",
    ...
    local welcome_phrase_2 = allphrases_details_2[math.random(1, #allphrases_details_2)]
    local avatars_random = {
        "391f98a3c2e8d09cb34bddac40f796cc",
        "3e654d51c838a86b659bfa58c6590eaa",
        "bc9321693caea9cf5aaf2b1988c084b8",
        "d3911e76ff36aed3ad290325fdb833be",
        "vzt-aoiskwe",
        "ee05a738393830e8ca79cf616dd9d4d8",
        "cccc298b30511268884a352e8cfaed5d",
        "cbj_gfmz4i8",
        "artworks-yjywznr2ftmxdjtg-fxesla-t500x500",
        "artworks-tw11n5xid9yoyaf5-uwk2oq-t500x500",
        "artworks-sfyryfe3hoeruxh2-icuc7g-t500x500",
        "artworks-qlsnsoyq2nmyjhre-h33wsa-t500x500",
        "artworks-mxnmbhmi2ffczfgi-eojryg-t500x500",
        "artworks-mpwdmskvdetu4o6y-wvtm1w-t500x500",
        "artworks-fsgt8zwzuad9xh5c-fom1nq-t500x500",
        "artworks-3rxfhkkudlcxfi0z-xbzpyg-t500x500",
        "94e1241db44df83acc41e44f6315f1db",
        "561746706b9b4d7536efb7104e99d8fa"
    }
    local randomavatars = avatars_random[math.random(1, #avatars_random)]
    presence.state = "~ " .. welcome_phrase
    presence.details = "~~ " .. welcome_phrase_2
    presence.largeImageKey = randomavatars 
    presence.largeImageText = "angelwave.lua - dsc.gg/cxrsedtears"
    discordRPC.Discord_Initialize("1211043911233966150", presence, 1, nil)
    discordRPC.Discord_UpdatePresence(presence)
    client.add_callback("unload", function() discordRPC.Discord_Shutdown() end)
end
...
_G.angelwave_push=(function()
	...
	function a:set_callback()
		...
		client.add_callback("render", function()
			...
			local l={b}
			local m={math.floor(render.measure_text(1,"[angelwave]  "..j.draw).x *1.03)}
			local n={render.measure_text(1,"[angelwave]  ").x}
           ...
           elseif menu.visual.interface_mode:get() == 0 and not menu.visual.notify_condition:get(2) then
                  ...
                  render.text(1, vector(p[1]+m[1]/2-o[1]/2 - 2, p[2] - 12 + posw.y),menu.visual.interface_color:get():alpha_modulate(h),"dc", "[angelwave]  ")
    ...
    if #_G.angelwave_notify_cache == 0 and menu.visual.notifyx_x:get() then
        ...
        local measure1 = render.measure_text(1,"⚠ missed shot due to angelwave user").x /2
        ...
        if menu.visual.notify_condition:get(2) then
            ...
            render.circle(vector(main.screen_size.x/2 + measure1 + 10, pos.y + 35), color(25,25,25, 255 *(animation.alpha1/255)), 11)
            render.pop_clip_rect()
            render.text(1, vector(main.screen_size.x/2, pos.y + 28), color(255,255,255, animation.alpha),"dc", "⚠ missed shot due to angelwave user!")

        elseif menu.visual.interface_mode:get() == 0 then
            render.rect(vector(main.screen_size.x/2 - render.measure_text(1,"[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)").x /2 - 10, pos.y - 8), vector(main.screen_size.x/2 + render.measure_text(1,"[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)").x/2 + 10 - animation.anim, pos.y -6), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,animation.alpha))
            render.rect(vector(main.screen_size.x/2 - render.measure_text(1,"[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)").x /2 - 10, pos.y - 6), vector(main.screen_size.x/2 + render.measure_text(1,"[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)").x/2 + 10, pos.y +16), color(21,21,21,menu.visual.interface_color:get().a*(animation.alpha1/255)))
            render.text(1, vector(main.screen_size.x/2, pos.y - 2), color(255,255,255, animation.alpha),"dc", "[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)")
            
            render.rect(vector(main.screen_size.x/2 - render.measure_text(1,"[angelwave] missed shot due to angelwave user").x /2 - 10, pos.y + 25), vector(main.screen_size.x/2 + render.measure_text(1,"[angelwave] missed shot due to angelwave user").x/2 + 10 - animation.anim, pos.y +27), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,animation.alpha))    
            render.rect(vector(main.screen_size.x/2 - render.measure_text(1,"[angelwave] missed shot due to angelwave user").x /2 - 10, pos.y + 27), vector(main.screen_size.x/2 + render.measure_text(1,"[angelwave] missed shot due to angelwave user").x/2 + 10, pos.y +49), color(21,21,21, menu.visual.interface_color:get().a*(animation.alpha1/255)))
            render.text(1, vector(main.screen_size.x/2, pos.y + 31), color(255,255,255, animation.alpha),"dc", "[angelwave] missed shot due to angelwave user")
        elseif menu.visual.interface_mode:get() == 1 then
            render.rect(vector(main.screen_size.x/2 - render.measure_text(1,"[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)").x /2 - 10, pos.y - 8), vector(main.screen_size.x/2 + render.measure_text(1,"[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)").x/2 + 10 - animation.anim, pos.y -6), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,animation.alpha))
            render.gradient(vector(main.screen_size.x/2 - render.measure_text(1,"[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)").x /2 - 10, pos.y - 6), vector(main.screen_size.x/2 + render.measure_text(1,"[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)").x/2 + 10, pos.y +16), menu.visual.interface_color:get():alpha_modulate(menu.visual.interface_color:get().a*(animation.alpha1/255)), menu.visual.interface_color:get():alpha_modulate(menu.visual.interface_color:get().a*(animation.alpha1/255)), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,0), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,0))
            render.text(1, vector(main.screen_size.x/2, pos.y - 2), color(255,255,255, animation.alpha),"dc", "[angelwave] hit zxkilla's head for 1 damage (99 hp remaining)")
            render.rect(vector(main.screen_size.x/2 - render.measure_text(1,"[angelwave] missed shot due to angelwave user").x /2 - 10, pos.y + 25), vector(main.screen_size.x/2 + render.measure_text(1,"[angelwave] missed shot due to angelwave user").x/2 + 10 - animation.anim, pos.y +27), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,animation.alpha))
            render.gradient(vector(main.screen_size.x/2 - render.measure_text(1,"[angelwave] missed shot due to angelwave user").x /2 - 10, pos.y + 27), vector(main.screen_size.x/2 + render.measure_text(1,"[angelwave] missed shot due to angelwave user").x/2 + 10, pos.y +49), menu.visual.interface_color:get():alpha_modulate(menu.visual.interface_color:get().a*(animation.alpha1/255)), menu.visual.interface_color:get():alpha_modulate(menu.visual.interface_color:get().a*(animation.alpha1/255)), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,0), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,0))
            render.text(1, vector(main.screen_size.x/2, pos.y + 31), color(255,255,255, animation.alpha),"dc", "[angelwave] missed shot due to angelwave user")
        end
...
local widgets = {
    ...
    watermark = {
        render = function()
            ...
            local text = "angelwave | " .. main.steamname .. " | " .. system_time_string
            
            if globals.is_in_game then
                  local ping = net_channel:get_latency(0)*1000 
                  text = "angelwave | " .. main.steamname .. (net_channel:is_loopback() and " | loopback | " or " | delay: " .. math.floor(ping) .. "ms | ") .. system_time_string
            end
    ...
    panel = {
        render = function()
            ...
            if menu.visual.interface_panelposition:get() == 0 then
                ...
                render.text(fonts.verdana, vector(36, main.screen_size.y /2), color(205, 205, 205, 255), "d", "WAVE")
                enchantedTextEffect(fonts.verdana, 5,main.screen_size.y /2, color(205,205,205,255), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,255), "d", "ANGEL", 1)
            elseif menu.visual.interface_panelposition:get() == 1 then
                ...
                render.text(fonts.verdana, vector(info.build == 'stable' and main.screen_size.x - 76 or main.screen_size.x - 64, main.screen_size.y /2), color(205, 205, 205, 255), "d", "WAVE")
                enchantedTextEffect(fonts.verdana, info.build == 'stable' and main.screen_size.x - 107 or main.screen_size.x - 95, main.screen_size.y /2, color(205,205,205,255), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,255), "d", "ANGEL", 1)
            elseif menu.visual.interface_panelposition:get() == 2 then
                ...
                render.text(fonts.verdana, vector(main.screen_size.x /2 - 15, main.screen_size.y - 15), color(205, 205, 205, 255), "d", "WAVE")
                enchantedTextEffect(fonts.verdana, main.screen_size.x /2 - 46,main.screen_size.y - 15, color(205,205,205,255), color(menu.visual.interface_color:get().r,menu.visual.interface_color:get().g,menu.visual.interface_color:get().b,255), "d", "ANGEL", 1)
            end
        end
   },
   alternative_wigets = {
    ...
    defensive_indicator = {
        defensive_icon = render.load_image(http:get('https://cdn.discordapp.com/attachments/1206602437411078154/1210984633643307088/65da1730f046c_1708791633_65da1730f0444.png?ex=65ec8c53&is=65da1753&hm=b06c2b690a528d6550682cba075b3d67b6cb5f7f0402e0ca372d0950443edfa2&'), vector(19, 17)),
        render = function(self)
            ...
            render.texture(self.defensive_icon, vector(main.screen_size.x /2 - text_size_slowdown.x + text_size_slowdown.x /2 - 22, pos.y + 59), color():alpha_modulate(255 * (animation_defensive.alpha/255)))
    ...
    slowed_down_indicator = {
        slowed_icon = render.load_image(http:get('https://cdn.discordapp.com/attachments/1175050304467775498/1210924621516378132/s.png?ex=65ec546f&is=65d9df6f&hm=912a7ddc86e6f9a513e0e45c6417cf62119ef48da2c6342ecd54518a64217b8e&'), vector(19, 17)),
        render = function(self)
        if not (menu.visual.slowed_down:get() and globals.is_in_game) then return end
            ...
            if menu.visual.slowed_down_style:get() == 0 then
                ...
                render.texture(self.slowed_icon, vector(main.screen_size.x /2 - text_size_slowdown.x + text_size_slowdown.x /2 - 22, pos.y + 59), color():alpha_modulate(255 * (animation_slowdown.alpha/255)))
    ...
    indicators_classic = {
        render = function()
           ...
           if rage.get_exploit_charge() >= 1 then chargecolor = color(255, 255, 255, 255) end
            local text_size = render.measure_text(fonts.pixel, 'angelwave')
            ...
            --render.rect(vector(main.screen_size.x /2 + animation_offset - (text_size.x / 2), main.screen_size.y/2 + 25), vector(main.screen_size.x /2 + animation_offset + (text_size.x / 2), main.screen_size.y/2 + 25 + text_size.y), color())
              render.text(fonts.pixel, vector(main.screen_size.x /2 + animation_offset, main.screen_size.y/2 + 25), color(255,255,255, 255), "oc", "angelwave")
              render.text(fonts.pixel, vector(main.screen_size.x /2 + animation_offset, main.screen_size.y/2 + 25), color(255,255,255, 255), "oc", "angelwave")
              enchantedTextEffect(fonts.pixel, main.screen_size.x /2 - 23+ animation_offset,main.screen_size.y/2 + 25, color(40,40,40, 255), accent, "o", "angelwave",1.8)
        ...
        indicators_old = {
            render = function()
               ...
               animation_offset3 = 1 and animation_offset3 * anim_num or animation_offset3
                  ...
                  -- @main text
                  enchantedTextEffect(fonts.verdana, main.screen_size.x /2 - 25+ animation_offset,main.screen_size.y/2 + 25, color(25,25,25, 255), accent, "d", "angelwave", 1)
...
local trashtalk_phrases = {
    ":(",
    "однажды я стану счастливым/ когда нибудь.",
    "кровь из носа/ не чувствую лица",
    "только ненависть/",
    "эти деньги не делают лучше",
    "@dampness - angelwave",
    "curved mirrors - angelwave",
    "he doesn’t need you at all/ just accept it",
    "i do not feel pain at all ^^",
    "threethreethree",
    "about the painful - dsc.gg/crxsedtears",
    "​disparaître",
    "​unrelease24/ but i fucked up",
    "x_x",
    "две четверки на моей шее",
    "elephant in the room this room is the world",
    "i save u or kill myself",
    "definitely",
    "last game and cut my wrists",
    "failed",
    "it’s normal to be bored - dsc.gg/crxsedtears",
    "you won’t understand",
    "усни вместе со своими мечтами",
    "я бы хотел никогда не существовать в этом мире",
    "bad ending - dsc.gg/crxsedtears",
    "​sérénité - dsc.gg/crxsedtears"
}
...
--]]
utils.set_window_name(string.format('angelwave%s - @dampness', info.build ~= 'stable' and ' beta' or ''))
...
client.add_callback("render", function()
    ...
    xpcall(function()
        if is_PARASHA_executing then config_frequency = config_frequency + 1 if config_frequency >= 50 then cfg_settings = {} for i = 1, #menu.builder do if i ~= 0 then
            ...
            clipboard.set('~angelwave~'..base64:encode(PARASHA))
